/**
 * Print category requirements and UI metadata.
 * Mirrors: ruxstar-frontend-services/lib/print-requirements.ts
 */

import type { PrintCategory, PrintRequirementField } from '@/types/print';

const SELECT_KEYS = new Set(['size', 'material', 'printType', 'color']);

// Fallback requirements used when the API has not yet returned `requirements`.
const STATIC_REQUIREMENTS: Record<string, PrintRequirementField[]> = {
  tshirts: [
    ...
    { key: 'placement',   label: 'Print placement',  type: 'text',     required: true,  hint: '', placeholder: 'e.g. Front chest, full back, sleeve' },
    { key: 'designImage', label: 'Artwork',          type: 'file',     required: true,  hint: 'Upload your logo or graphic', placeholder: '' },
  ],
  hoodies: [
    ...
    { key: 'placement',   label: 'Print placement',      type: 'text',     required: false, hint: '', placeholder: 'Front, back, hood, sleeve…' },
  ...
  jerseys: [
    ...
    { key: 'teamName',      label: 'Team / club name',   type: 'text',     required: true,  hint: '', placeholder: 'Team or academy name' },
    { key: 'playerDetails', label: 'Names & numbers',    type: 'textarea', required: false, hint: '', placeholder: 'Player names, numbers, sizes (one per line)' },
    { key: 'designImage',   label: 'Team logo / design', type: 'file',     required: true,  hint: 'Upload logo and layout reference', placeholder: '' },
  ],
  business_cards: [
    ...
    { key: 'contactDetails', label: 'Contact info to print', type: 'textarea', required: true, hint: '', placeholder: 'Name, phone, email, address, website…' },
    { key: 'designImage',    label: 'Card design',           type: 'file',     required: true, hint: 'Upload front/back artwork', placeholder: '' },
  ],
  posters: [
    ...
    { key: 'finishing',   label: 'Finishing',         type: 'text',   required: false, hint: '', placeholder: 'Lamination, mounting, eyelets…' },
  ...
  flex_banners: [
    ...
    { key: 'customDimensions', label: 'Exact size (custom)', type: 'text',  required: false, hint: '', placeholder: 'Width × height in feet or metres' },
    { key: 'installation',     label: 'Installation needs', type: 'text',   required: false, hint: '', placeholder: 'Eyelets, pole pockets, rope, stand…' },
    { key: 'designImage',      label: 'Banner artwork',     type: 'file',   required: true,  hint: 'High-resolution file preferred', placeholder: '' },
  ],
  stickers: [
    ...
    { key: 'color',       label: 'Colour mode',      type: 'select', required: false, hint: '', placeholder: '' },
    { key: 'shape',       label: 'Shape / outline',  type: 'text',   required: false, hint: '', placeholder: 'Circle, rectangle, custom die-cut…' },
  ...
  invitations: [
    ...
    { key: 'eventDate',    label: 'Event date',     type: 'text',     required: true,  hint: '', placeholder: 'Date and time of event' },
    { key: 'eventDetails', label: 'Event wording',  type: 'textarea', required: false, hint: '', placeholder: 'Names, venue, RSVP line…' },
  ...
  photo_prints: [
    ...
    { key: 'cropNotes',   label: 'Crop / border instructions', type: 'text',   required: false, hint: '', placeholder: 'Full bleed, white border, crop area…' },
    { key: 'designImage', label: 'Photo file',                 type: 'file',   required: true,  hint: 'Upload the photo to print', placeholder: '' },
  ],
  documents: [
    ...
    { key: 'pageCount',   label: 'Number of pages',   type: 'text',   required: true,  hint: '', placeholder: 'Total pages to print' },
    { key: 'designImage', label: 'Document scan',     type: 'file',   required: false, hint: 'Optional — share files with the vendor after accept', placeholder: '' },
  ],
};
...
export function requirementOptions(category: PrintCategory, key: string): string[] {
  switch (key) {
    case 'size':      return category.sizes;
    case 'material':  return category.materials;
    case 'printType': return category.printTypes;
    case 'color':     return category.colorOptions;
    default:          return [];
  }
}

export function isSelectRequirement(field: PrintRequirementField): boolean {
